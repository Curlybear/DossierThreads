...
while(1) {
    ...
    switch(Requete.requete) {
        case LOGIN :
        Trace("Connection Client : pid=%ld login=%s pseudo=%s",Requete.client,Requete.login,Requete.pseudo);
        if (ajouteClient(Requete.client,Requete.login,Requete.pseudo) == 0) Reponse.ok = 1;
        else Reponse.ok = 0;
        break;

        case SCORE :
        Trace("Reception Score : pid=%ld score=%d",Requete.client,Requete.score);
        if (Requete.score > TopScore.score) {
            ...
            Reponse.topScoreBattu = 0;
        }
        Reponse.ok = 1;
        break;

        case LOGOUT :
        Trace("Deconnection Client : pid=%ld",Requete.client);
        fflush(stdout);
        if (supprimeClient(Requete.client) == 0) {
            Reponse.ok = 1;
        } else {
            Reponse.ok = 0;
        }
